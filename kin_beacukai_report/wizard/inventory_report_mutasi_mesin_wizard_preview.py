...
class InventoryPreviewMutasiMesinReportWizard(models.TransientModel):
    ...
    location_ids = fields.Many2many('stock.location', 'ms_stock_mutasi_mesin_location_rel', 'stock_mutasi_id',
                                    'location_id', 'Lokasi', copy=False, domain=[('usage', '=', 'internal')])
    categ_ids = fields.Many2many('product.category', 'ms_stock_mutasi_mesin_categ_rel', 'stock_mutasi_id',
                                 'categ_id', 'Kategori Produk', copy=False)
    product_ids = fields.Many2many('product.product', 'ms_stock_mutasi_mesin_product_rel', 'stock_mutasi_id',
        ...
        query = """
                ...
                from product_product pp
                    ...
                    left join
                        (
                            select sm.product_id, 
                            sum(case when sl.complete_name = 'SMK/Stock' and dl.complete_name != 'SMK/Stock' and inventory_id is not null
                                then sm.product_qty*-1 
                            when sl.complete_name != 'SMK/Stock' and dl.complete_name = 'SMK/Stock' and inventory_id is not null
                                then sm.product_qty else 0 end ) as qty
    ...
    def print_inventory_preview_report(self):
        return self.env.ref('kin_beacukai_report.report_inventory_mutasi_mesin').report_action(self)
