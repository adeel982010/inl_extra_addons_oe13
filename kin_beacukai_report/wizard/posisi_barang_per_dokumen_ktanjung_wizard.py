...
class PosisiBarangPerDokumenKtanjungExportSummary(models.TransientModel):
    _name = "posisi.barang.per.dokumen.ktanjung.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char("Name", size=32, default='laporan_posisi_barang_per_dok_ktanjung.xls')


class PosisiBarangPerDokumenKtanjungWizard(models.TransientModel):
    _name = 'posisi.barang.per.dokumen.ktanjung.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        for val in vals:
            ...
            worksheet.write(row, 4, no_invoice, xls_format.font_style(
                position='center', border=1, fontos='black', font_height=200, color='false'))
            worksheet.write(row, 5, tgl_invoice, xls_format.font_style(
                position='center', border=1, fontos='black', font_height=200, color='false'))
        ...
        res_id = self.env['posisi.barang.per.dokumen.ktanjung.export.summary'].create(
            {'name': 'Laporan Posisi Barang PerDok (Kuala Tanjung).xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=posisi.barang.per.dokumen.ktanjung.export.summary&field=file&id=%s&filename=Laporan Posisi Barang PerDok (Kuala Tanjung).xls' % (res_id.id),
            'target': 'new',
        }