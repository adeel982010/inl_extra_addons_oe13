...
class InventoryExcelMutasiKtanjungExportSummary(models.TransientModel):
    _name = "inventory.excel.mutasi.ktanjung.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char(
        "Name", size=32, default='Laporan Mutasi Barang Kuala Tanjung.xls')


class InventoryExportMutasiKtanjungReportWizard(models.TransientModel):
    _name = 'inventory.export.mutasi.ktanjung.report.wizard'
    ...
    location_ids = fields.Many2many('stock.location', 'mutasi_ktanjung_location_rel', 'stock_mutasi_id',
                                    'location_id', 'Lokasi', copy=False, domain=[('usage', '=', 'internal')])
    categ_ids = fields.Many2many('product.category', 'mutasi_ktanjung_categ_rel', 'stock_mutasi_id',
                                 'categ_id', 'Kategori Produk', copy=False)
    product_ids = fields.Many2many('product.product', 'mutasi_ktanjung_product_rel', 'stock_mutasi_id',
    ...
    def print_inventory_export_report(self):
        ...
        where_location_opn = " sil.location_id = 29 "
        ...
        query = """
                ...
                from product_product pp
                    left join
                        (
                            ...
                            sum(case when sl.complete_name = 'KTJ/Stock' and dl.complete_name != 'KTJ/Stock'
                                then sm.product_qty*-1 
                            when sl.complete_name != 'KTJ/Stock' and dl.complete_name = 'KTJ/Stock'
                    ...
                    left join 
                        (
                            ...
                            left join product_template pt on pt.id=pp.product_tmpl_id
                            left join uom_uom pu on pu.id= sm.product_uom
                            where sl.complete_name != 'KTJ/Stock' and dl.complete_name = 'KTJ/Stock' and sm.state = 'done' and sm.inventory_id is null
                    ...
                    left join
                        (
                            select sm.product_id, 
                            sum(case when sl.complete_name = 'KTJ/Stock' and dl.complete_name != 'KTJ/Stock' and inventory_id is not null
                                then sm.product_qty*-1 
                            when sl.complete_name != 'KTJ/Stock' and dl.complete_name = 'KTJ/Stock' and inventory_id is not null
                                then sm.product_qty else 0 end ) as qty
                ...
                where """ + where_categ + """ and """ + where_product + """ 
                    and pc.report_type IN ('bahan_baku','barang_jadi')
                    and pp.active = 't'
        ...
        res_id = self.env['inventory.excel.mutasi.ktanjung.export.summary'].create(
            {'name': 'Laporan Mutasi Barang Kuala Tanjung.xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.mutasi.ktanjung.export.summary&field=file&id=%s&filename=Laporan Mutasi Barang Kuala Tanjung.xls' % (res_id.id),
            'target': 'new',
        }
