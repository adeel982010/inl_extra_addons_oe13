...
class PosisiStokBarangBelawanExportSummary(models.TransientModel):
    _name = "posisi.stok.barang.belawan.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char("Name", size=32, default='laporan_posisi_stok_barang_belawan.xls')


class PosisiStokBarangBelawanWizard(models.TransientModel):
    _name = 'posisi.stok.barang.belawan.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        res_id = self.env['posisi.stok.barang.belawan.export.summary'].create(
            {'name': 'Laporan Posisi Stok Barang (Belawan).xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=posisi.stok.barang.belawan.export.summary&field=file&id=%s&filename=Laporan Posisi Stok Barang (Belawan).xls' % (res_id.id),
            'target': 'new',
        }