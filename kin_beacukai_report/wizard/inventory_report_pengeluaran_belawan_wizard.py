...
class InventoryExcelPengeluaranBelawanExportSummary(models.TransientModel):
    _name = "inventory.excel.pengeluaran.belawan.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char("Name", size=32, default='laporan_pengeluaran_belawan.xls')


class InventoryExportPengeluaranBelawanReportWizard(models.TransientModel):
    _name = 'inventory.export.pengeluaran.belawan.report.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        res_id = self.env['inventory.excel.pengeluaran.belawan.export.summary'].create(
            {'name': 'Laporan Pengeluaran Barang(Belawan).xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.pengeluaran.belawan.export.summary&field=file&id=%s&filename=Laporan Pengeluaran Barang(Belawan).xls' % (res_id.id),
            'target': 'new',
        }