...
class AktivitasImportirBelawanExportSummary(models.TransientModel):
    _name = "aktivitas.importir.belawan.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char("Name", size=32, default='laporan_aktivitas_importir_belawan.xls')


class AktivitasImportirBelawanWizard(models.TransientModel):
    _name = 'aktivitas.importir.belawan.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        worksheet.write_merge(3, 3, 0, 4, "LAPORAN AKTIVITAS IMPORTIR " + str(
        ...
        res_id = self.env['aktivitas.importir.belawan.export.summary'].create(
            {'name': 'Laporan Aktivitas Importir (Belawan).xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=aktivitas.importir.belawan.export.summary&field=file&id=%s&filename=Laporan Aktivitas Importir (Belawan).xls' % (res_id.id),
            'target': 'new',
        }