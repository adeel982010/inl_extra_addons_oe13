...
class InventoryPreviewMutasiJadiReportWizard(models.TransientModel):
    ...
    location_ids = fields.Many2many('stock.location', 'ms_stock_mutasi_jadi_location_rel', 'stock_mutasi_id',
                                    'location_id', 'Lokasi', copy=False, domain=[('usage', '=', 'internal')])
    categ_ids = fields.Many2many('product.category', 'ms_stock_mutasi_jadi_categ_rel', 'stock_mutasi_id',
                                 'categ_id', 'Kategori Produk', copy=False)
    product_ids = fields.Many2many('product.product', 'ms_stock_mutasi_jadi_product_rel', 'stock_mutasi_id',
                ...
                from product_product pp
                    left join
                        (
                            ...
                            sum(case when sl.complete_name = 'SMK/Stock' and dl.complete_name != 'SMK/Stock'
                                then sm.product_uom_qty*-1 
                            when sl.complete_name != 'SMK/Stock' and dl.complete_name = 'SMK/Stock'
                    ...
                    left join 
                        (
                            ...
                            left join product_template pt on pt.id=pp.product_tmpl_id
                            left join uom_uom pu on pu.id= sm.product_uom
                            where sl.complete_name != 'SMK/Stock' and dl.complete_name = 'SMK/Stock' and sm.state = 'done' and sm.inventory_id is null
                    ...
                    left join
                        (
                            select sm.product_id, 
                            sum(case when sl.complete_name LIKE 'SMK/Stock%' and dl.complete_name NOT LIKE 'SMK/Stock%' and inventory_id is not null
                                then sm.product_qty*-1 
                            when sl.complete_name NOT LIKE 'SMK/Stock%' and dl.complete_name LIKE 'SMK/Stock%' and inventory_id is not null
                                then sm.product_uom_qty else 0 end ) as qty
    ...
    def print_inventory_preview_report(self):
        return self.env.ref('kin_beacukai_report.report_inventory_mutasi_jadi').report_action(self)
