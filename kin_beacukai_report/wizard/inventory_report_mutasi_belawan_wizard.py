...
class InventoryExcelMutasiBelawanExportSummary(models.TransientModel):
    _name = "inventory.excel.mutasi.belawan.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char(
        "Name", size=32, default='Laporan Mutasi Barang Belawan.xls')


class InventoryExportMutasiBelawanReportWizard(models.TransientModel):
    _name = 'inventory.export.mutasi.belawan.report.wizard'
    ...
    location_ids = fields.Many2many('stock.location', 'mutasi_belawan_location_rel', 'stock_mutasi_id',
                                    'location_id', 'Lokasi', copy=False, domain=[('usage', '=', 'internal')])
    categ_ids = fields.Many2many('product.category', 'mutasi_belawan_categ_rel', 'stock_mutasi_id',
                                 'categ_id', 'Kategori Produk', copy=False)
    product_ids = fields.Many2many('product.product', 'mutasi_belawan_product_rel', 'stock_mutasi_id',
    ...
    def print_inventory_export_report(self):
                ...
                from product_product pp
                    left join
                        (
                            ...
                            sum(case when sl.complete_name = 'BLW/Stock' and dl.complete_name != 'BLW/Stock'
                                then sm.product_qty*-1 
                            when sl.complete_name != 'BLW/Stock' and dl.complete_name = 'BLW/Stock'
                    ...
                    left join 
                        (
                            ...
                            left join product_template pt on pt.id=pp.product_tmpl_id
                            left join uom_uom pu on pu.id= sm.product_uom
                            where sl.complete_name != 'BLW/Stock' and dl.complete_name = 'BLW/Stock' and sm.state = 'done' and sm.inventory_id is null
                    ...
                    left join
                        (
                            select sm.product_id, 
                            sum(case when sl.complete_name = 'BLW/Stock' and dl.complete_name != 'BLW/Stock' and inventory_id is not null
                                then sm.product_qty*-1 
                            when sl.complete_name != 'BLW/Stock' and dl.complete_name = 'BLW/Stock' and inventory_id is not null
                                then sm.product_qty else 0 end ) as qty
        ...
        res_id = self.env['inventory.excel.mutasi.belawan.export.summary'].create(
            {'name': 'Laporan Mutasi Barang Belawan.xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.mutasi.belawan.export.summary&field=file&id=%s&filename=Laporan Mutasi Barang Belawan.xls' % (res_id.id),
            'target': 'new',
        }
